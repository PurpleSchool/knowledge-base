...
### Пример: строка в формате `DD.MM.YYYY`

Частый кейс - даты вида `21.03.1989`. В Go формат задаётся через «эталонную дату» **02.01.2006** (день=02, месяц=01, год=2006), поэтому для `DD.MM.YYYY` нужен layout `"02.01.2006"`:
...
func main() {
    ...
    // Вариант 1: если вам подходит UTC
    t, err := time.Parse("02.01.2006", s)
    if err != nil {
        panic(err)
    }
    fmt.Println(t) // 1989-03-21 00:00:00 +0000 UTC

    // Вариант 2: если важна локальная таймзона (например, Europe/Moscow)
    loc, _ := time.LoadLocation("Europe/Moscow")
    tLocal, err := time.ParseInLocation("02.01.2006", s, loc)
    if err != nil {
        panic(err)
    }
    fmt.Println(tLocal)
