...
## Обзор полезных gradle-плагинов для React Native

Ниже вы найдете перечень gradle-плагинов, которые часто требуются в React Native-проектах:

| Плагин            | Назначение                               |
|-------------------|------------------------------------------|
| google-services   | Интеграция Firebase, Google API          |
| crashlytics       | Сбор и отправка crash-логов (Firebase)   |
| kotlinx-android   | Поддержка Kotlin                         |
| realm-android     | Использование Realm DB                    |
| sentry-android    | Интеграция Sentry для отчетов об ошибках |
| com.onesignal     | Push-уведомления через OneSignal         |

Для каждого такого плагина сценарий подключения всегда похож: добавить classpath-зависимость в project build.gradle и применить плагин через строку `apply plugin` или новый синтаксис `plugins { id ... }` в module build.gradle.
