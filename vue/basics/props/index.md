...
## Использование props вместе со slot'ами

Важно не путать направления передачи данных:

- **Props**: родитель → ребёнок (родитель «кормит» компонент данными).
- **Slots**: родитель → ребёнок (родитель «кормит» компонент разметкой/шаблоном).
- **Scoped slots (slot props)**: ребёнок → родитель (ребёнок «делится данными» с тем шаблоном, который родитель передал в слот).

### Что значит «передать данные в слот»

Фраза «передать данные слоту» обычно означает именно **slot props**: компонент, внутри которого объявлен `<slot>`, может **привязать к нему данные** (`:item="..."`), а родитель **получит эти данные** в `template #slotName="{ ... }"`.

Если же вы хотите «передать данные от родителя в слот», то это делается автоматически: слот- шаблон находится в контексте родителя и **видит переменные родителя** напрямую.

### Пример scoped slot (ребёнок отдаёт данные в слот)
...
Теперь контент в родителе получает доступ к каждому `item` через **slot props**.

Подробнее про слоты и scoped slots (с ошибками/типовыми кейсами): см. статью `vue/components/slots`.
