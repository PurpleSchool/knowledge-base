...
- **SSR (Nuxt 3 и Nuxt 4):**
  ...
  ```bash
  # Сборка production-бандла — результат попадёт в папку .output
  npm run build

  # Запуск собранного приложения в production-режиме
  node .output/server/index.mjs
  ```

  Локально посмотреть, как ведёт себя production-сборка, можно через встроенный превью-сервер:

  ```bash
  # nuxt preview поднимает собранное приложение на локальной машине
  npm run preview
  ```
...
- **SSR (Nuxt 2):**
  ...
  ```bash
  # В Nuxt 2 за запуск production-сервера отвечает команда nuxt start
  npm run build
  npm run start
  ```
...
- `npm run build` — собирает приложение, оптимизируя скрипты, CSS, производит tree shaking. Для SSR и SSG выполняет подготовку серверной части. В Nuxt 3 и Nuxt 4 результат сборки складывается в папку `.output`.
- `npm run generate` — генерирует полностью статический сайт, если используется режим SSG.
- `node .output/server/index.mjs` — запускает production-сервер в Nuxt 3 и Nuxt 4. После этого приложение доступно на выбранном порте (по умолчанию 3000).
- `npm run preview` — поднимает собранное приложение локально для проверки перед деплоем. Это инструмент разработчика, для боевого сервера он не предназначен.
- `npm run start` — production-сервер в Nuxt 2. Обратите внимание: в Nuxt 3 и Nuxt 4 такого скрипта в стандартном шаблоне нет, и эта команда приложение не запустит.
...
В Nuxt 3 и Nuxt 4 боевой сервер запускается напрямую из собранного бандла — отдельный npm-скрипт для этого не нужен.

```bash
# Nuxt 3 / Nuxt 4 — запуск production-сервера
node .output/server/index.mjs
# Стартует на порте 3000 по умолчанию
```

Порт и хост задаются переменными окружения:

```bash
PORT=8080 HOST=0.0.0.0 node .output/server/index.mjs
```

В Nuxt 2 для этой задачи используется `npm run start`.
...
В Nuxt 3 и Nuxt 4 порт задаётся переменной окружения при запуске собранного бандла:
```bash
PORT=4000 node .output/server/index.mjs
```
Переменную можно вынести и в `.env`. В Nuxt 2 для этого использовались `PORT=4000 npm run start` или опция `nuxt start -p 4000`.
