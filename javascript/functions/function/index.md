...
Обратите внимание, что можно вызывать функцию столько раз, сколько будет желание. Можно написать одну функцию, а затем вызывать ее несколько раз с разными аргументами.
...
- Функция делает код многоразовым. Вы можете объявить ее один раз и использовать несколько раз.
...
> **Примечание:** Следует избегать использования глобальных переменных, поскольку значение глобальной переменной может изменяться в различных областях программы. Это может привести к неизвестным результатам в программе.
...
Следовательно, в приведенной выше программе первые два `console.log()` работают без проблем.

Однако мы пытаемся получить доступ к переменной `c`, скопированной в блок, вне блока в третьей строке `console.log()`. Это приведет к ошибке.

> **Примечание:** В JavaScript переменная `var` относится к функциональной scope, а `let` - к блочным scope. Если попытаться использовать `var c = 'hello';` внутри оператора `if` в приведенной выше программе, вся программа будет работать, так как `c` будет рассматриваться как локальная переменная.
