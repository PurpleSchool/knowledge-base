...
metaDescription: Откройте для себя, что такое props в React - как они помогают передавать данные между компонентами и как правильно их применять для построения гибких...
author: Олег Марков
...
- Упорядоченными (данные передаются от родителя к потомкам)
...
### Что означает «поток данных сверху вниз» (top-down)

В React интерфейс - это **дерево компонентов**.

- **«Верх»** - компонент, который *рендерит* другой компонент (родитель).
- **«Низ»** - компонент, который *вложен* и получен в JSX родителя (ребёнок/потомок).

То есть «данные сверху вниз» означает: **state живёт выше (в родителе), а в детей передаётся через props**.

Пример дерева:

```txt
App (держит state)
└── Profile (получает props)
    └── Avatar (получает props)
```

Обратное направление («снизу вверх») достигается не изменением props, а **колбэками**: родитель передаёт в ребёнка функцию, ребёнок вызывает её, и родитель обновляет свой state.
...
### Как «изменять» данные родителя из дочернего компонента

Дочерний компонент не должен менять `props` напрямую. Но он может **попросить родителя изменить state**, если родитель передаст вниз функцию (callback) через props.

Это базовый паттерн React: **state хранится выше (lifting state up)**, а дочерние компоненты получают данные и события.

Пример: дочерний компонент меняет значение в родителе через `onChangeName`:
...
function Child({ name, onChangeName }) {
  return (
    <div>
      ...
      <button onClick={() => onChangeName('Антон')}>Сменить имя</button>
    </div>
  );
}
...
#### Почему «локальная копия props» в дочернем не меняет родителя
...
то `localName` - это **отдельный state** ребёнка. Он:
- не изменяет `name` в родителе
- и обычно **не будет обновляться**, когда родитель передаст новый `name` (инициализация `useState(name)` выполняется только при первом рендере)

Локальная копия имеет смысл только если вам нужен «черновик» (например, редактирование формы с кнопкой “Сохранить”). Тогда при сохранении вы явно вызываете callback, который меняет state в родителе.
