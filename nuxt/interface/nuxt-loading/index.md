...
В Nuxt 3 подход изменился: настройка `loading` в `nuxt.config` больше не используется, а вместо неё появился встроенный компонент `<NuxtLoadingIndicator>`. Устанавливать для этого ничего не нужно — компонент доступен из коробки благодаря автоимпорту.

### Встроенный компонент NuxtLoadingIndicator

Достаточно добавить его в `app.vue` или в layout:
...
Прогресс-бар появится в верхней части экрана при переходах между страницами и исчезнет, когда навигация завершится.

Внешний вид и поведение настраиваются пропсами:
...
Разберём основные пропсы:

- `color` — цвет полосы. Можно передать CSS-градиент или `false`, чтобы отключить стандартную заливку и задать оформление своими стилями;
- `height` — высота полосы в пикселях (по умолчанию `3`);
- `duration` — ожидаемая длительность загрузки в миллисекундах, влияет на скорость заполнения полосы;
- `throttle` — задержка перед появлением индикатора. Нужна, чтобы полоса не мигала на быстрых переходах.

Если требуется управлять индикатором вручную, воспользуйтесь composable `useLoadingIndicator`:

```js
const { start, finish, clear, isLoading, progress } = useLoadingIndicator()

// Запускаем индикатор вручную — например, перед долгой операцией
start()

// И завершаем, когда операция закончилась
finish()
```
